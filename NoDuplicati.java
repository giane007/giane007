import java.util.Scanner; //tipo di package
import java.util.Random;
import java.util.HashSet;
import java.util.*; //insieme di package
...
public class NonDuplicati {
    ...
    private static int[] numeriCasuali(int n) {
        ...
        return numeriUnici.stream().mapToInt(Integer::intValue).toArray();
    }

    private static int[] filtraPari(int[] numeri) {
        return Arrays.stream(numeri).filter(num -> num % 2 == 0).toArray();
    }
